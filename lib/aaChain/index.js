 ...
 * 创建交易体
 */
EthSdk.prototype.createTxInfo = async function ({ from, to, value, fee, data = '0x', nonce, rate }) {
    if (nonce == undefined) {
        throw '请传入nonce'
    }
    // 验证地址是否有效
    ...
    var bigGasPrice = new Bignumber(gasPrice).times(rate).decimalPlaces(0)
    var gas = 0
    if (rate === 1) {
        gas = new Bignumber(fee).div(gasPrice).decimalPlaces(0)
    } else {
        gas = new Bignumber(fee).div(gasPrice).times(rate).decimalPlaces(0)
    }
    // 获取nonce
    // var nonce = await this.web3.eth.getTransactionCount(from, 'pending');
    // 创建一个交易体
    console.log('xxxxxxx cre')
...
EthSdk.prototype.getErc20TransferFee = async function (contract, from, recipient, amount) {
    // var contract = this.web3.eth.contract(abi).at(contract);
    // var token = new this.web3.eth.Contract(abi, contract);

    // let data = token.methods.transfer.getData(recipient, amount);
    // return data;
    var bigAmount = new Bignumber(amount);
    var token = new this.web3.eth.Contract(abi, contract);
    var data = token.methods.transfer(recipient, bigAmount.toString(10)).encodeABI();
...
EthSdk.prototype.createTx = async function (privateKey, recipientAddress, amount, fee, contract, data, nonce, rate = 1) {
...
EthSdk.prototype.getErc20InfoFromInput = function (tx) {
    if (tx.input.slice(2, 10) !== "a9059cbb") {
        return {
            erc20From: '',
            erc20To: '',
            erc20Value: '0',
    ...
    let to = "0x" + tx.input.slice(10, 74);
    to = new Bignumber(to).toString(16);
    let value = "0x" + tx.input.slice(75, 139);
...
EthSdk.prototype.getErc20TxArr = async function (tx) {
    let defaultTx = Object.assign({
        erc20From: '',
        erc20To: '',
        erc20Value: '0',
        contract: '',
    ...
    var recipientTx = {}
    try {
        recipientTx = await this.web3.eth.getTransactionReceipt(tx.hash)

    } catch (_) {
        return [defaultTx];
    }
    var logs = recipientTx.logs;

    if (!logs || !logs.length) {
        // if (txData.input.slice(2, 10) == "a9059cbb") {
        //     let to = "0x" + txData.input.slice(10, 74);
        //     let value = "0x" + txData.input.slice(75, 139);
        //     tx.Erc20Value = new Bignumber(value).toString(10);
        //     tx.to = "0x" + new Bignumber(to).toString(16);
        //     tx.erc20From = txData.from
    ...
    for (var i = 0; i < logs.length; i++) {
        ...
        if (topics && topics.length == 3) {
            ...
            if (topics[0] === TransferMethodSha) {
                ...
                let _tx = Object.assign({
                    erc20From: from,
                    erc20To: to,
                    erc20Value: (new Bignumber(log.data)).toString(10),
...
EthSdk.prototype.getAverageFee = async function (to, data) {
    ...
    var gas = '21000';
    // var gas = await this.web3.eth.estimateGas({
    //     from: "0xb4D059Ca1c63B6cd90D899aDE1837FC50fA10F39",
    //     to,
    //     data
    // });
    var gasPrice = await this.web3.eth.getGasPrice()
